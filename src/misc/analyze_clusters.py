...
def get_latent_vectors(dataset):
    vae = VAEVerbClasses(dataset)
    V, N, P = vae.get_V_N_P()

    with tf.Session() as sess:
        summary_writer = tf.summary.FileWriter('../out/tf/experiment',
                                              graph=sess.graph)
        if os.path.isfile("../out/tf/vae-model.ckpt"):
            print("Restoring saved parameters")
            saver.restore(sess, "../out/tf/vae-model.ckpt")
        else:
        ...
        for batch in list(chunks(ys, batch_size)):
            ...
            feed_dict = {vae.V: vs, vae.N: ns, vae.P: ps, vae.include_VP: 1}
            output = sess.run( vae.z , feed_dict=feed_dict)
...
def k_means(l_vectors,ys, n_clusters):
    print("starts kmeans clusterting...")

    kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(l_vectors)
    print("kmeans has been fitted")

    ys_clusters = defaultdict(lambda: defaultdict(int))
    for i, (vp, n, v, p) in enumerate(ys):
        ys_clusters[ int(kmeans.labels_[i])][v] += 1
...
def determine_ys_cluster(l_vectors, ys):
    ys_clusters = defaultdict(lambda: defaultdict(int))
    for  i,(vp,n,v,p) in enumerate(ys):
        ys_clusters[int(l_vectors[i].argmax(axis=50))][v] += 1

    return ys_clusters


def create_final_clusters(clusters, dataset, use_softmax = True, min_freq=1000):
...
if __name__ == "__main__":
    ...
    if make_new_latent_vectors:
        ...
        print("loaded latent_vectors")

    # #take max of the latent vector to find the cluster
    # # clusters = create_final_clusters(determine_ys_cluster(latent_vectors, ys), dataset)
    #
    # #cluster with kmeans
    # clusters = create_final_clusters( k_means(latent_vectors, ys,50 ), dataset)
    # print("clusters have been created")
    # pickle.dump(clusters, open("../out/clusters.p", "wb"))
    #
    # print_clusters(clusters, 10)
