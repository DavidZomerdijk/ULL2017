...
def make_table_rows(dataset, data_model, cls):
    ...
    for v_idx in range(20):
        ...
        row = '\\' + " {0:.3f}".format(data_model.p_vc[vp_idx,cls]) + ' & \\verb|' + str('.'.join(vp)) + "|  "
...
def make_top_rows(data_model, cls=0):
    ...
    row1 = "\hline \\begin{tabular}[c]{@{}l@{}}Class " + str(cls + 1) + "  \\end{tabular} & $\\theta_{nc}$ "
    row2 = " $\\theta_{vc}$ & $\\theta_{c} = %.3f$ " % (prob,)
    for n_idx in range(20):
        noun_idx = p_nc_c_idx[n_idx]
        row1 += "& \\rotatebox[origin=c]{90}{ %.3f } " % (data_model.p_nc[noun_idx, cls],)
